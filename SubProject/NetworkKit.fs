    ...
    module NetWork =
        ...
        type SimpleNetWork (weights:Matrix.Matrix2D[],deflections:Matrix.Matrix2D[]) =
            ...
            member private this.JudgeFunc (input:Matrix.Matrix2D) (stack:int) =
                ...
                then this.JudgeFunc ((Matrix.dot input this.weights.[stack].T).map sigmoid) (stack+1) //Warning : First input get sigmoid
                else input
            member this.GetJudgeValues (input:Matrix.Matrix2D) = this.JudgeFunc input 0
